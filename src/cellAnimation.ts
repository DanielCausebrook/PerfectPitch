...
export class CellAnimation {
    ...
    static #rotationAxis(direction: RectDirection) {
        return getRectUnitVector(rotateRectDirection(direction, 2));
    }
    ...
    static wobble(direction: RectDirection, initialRotation: number, durationMs: number): CellAnimation {
        const strikeOffset = 0.05;
        const decay = 0.625;
        const numWobbles = 6;
        const axis = this.#rotationAxis(direction);
        let animation: Keyframe[] = [
            {transform: `rotate3d(${axis.x}, ${-axis.y}, 0, 0deg)`, easing: 'ease-out'},
            {
                transform: `rotate3d(${axis.x}, ${-axis.y}, 0, ${initialRotation}deg)`,
                offset: strikeOffset,
        ...
        for (let i = 1; i < numWobbles; i++) {
            animation.push({
                transform: `rotate3d(${axis.x}, ${-axis.y}, 0, ${initialRotation * Math.pow(-decay, i)}deg)`,
                offset: strikeOffset + i * (1 - strikeOffset) / numWobbles,
                easing: `cubic-bezier(.42,0,.58,1)`,
            });
        }
        animation.push({transform: `rotate3d(${axis.x}, ${-axis.y}, 0, 0deg)`})
        return new CellAnimation([
            new CellAnimationComponent(animation, durationMs),
        ])
    }

    static spin(direction: RectDirection, durationMs: number): CellAnimation {
        let axis = this.#rotationAxis(direction);
        return new CellAnimation([
            new CellAnimationComponent([
                {transform: `rotate3d(${axis.x}, ${-axis.y}, 0, 0deg)`, easing: 'cubic-bezier(0, 0.2, 0, 1)'},
                {transform: `rotate3d(${axis.x}, ${-axis.y}, 0, ${360 * 4}deg)`},
            ], durationMs),
        ])
    }
}

export function createInteractAnimation(color: string, intensity: number, direction: RectDirection): CellAnimation {
    return CellAnimation.combine(
        CellAnimation.wobble(direction, 75*intensity, 1000),
        CellAnimation.glow(color, 15*intensity, 1000)
    );
}
export function createSinkAnimation(color: string, direction: RectDirection): CellAnimation {
    return CellAnimation.combine(
...
export function playWinAnimation(center: RectPoint2D, cells: RectTiling2D<HTMLElement>): Promise<void> {
    ...
    cells.forEach((cell, p) => {
        ...
        let animation = [
            {transform: `rotate3d(${tangent.x}, ${tangent.y}, 0, 0deg)`},
            {transform: `rotate3d(${tangent.x}, ${tangent.y}, 0, 0deg)`, easing: `cubic-bezier(${0.12 * adjustmentAmount}, ${1 + adjustmentAmount * 1.65}, ${0.8 - 0.35*adjustmentAmount}, ${1 - adjustmentAmount*0.5})`, offset: startOffset*0.9},
            {transform: `rotate3d(${tangent.x}, ${tangent.y}, 0, ${end}deg)`, offset: 0.9},
            {transform: `rotate3d(${tangent.x}, ${tangent.y}, 0, ${end}deg)`},
        ];
        const timing = {
            duration: duration,
            iterations: 1,
        };
        cell.animate(animation, timing);
    })
    return timeout(duration);
}